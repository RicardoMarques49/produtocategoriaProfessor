...
def selecionar(motor):
    """
    Retorna o id de uma produto selecionada
    : param o motor: motor de acesso ao banco
...
def alterar(motor):
    ...
    with Session(motor) as sessao:
        ...
        novo_preço = input(f"Qual o novo preço ({produto.preco:.2f})? ")
        situacao_hoje = "Ativo" if produto.ativo else "Inativo"
        novo_ativo = input(f" Muda a status do produto ({situacao_hoje}) (S/N)? ")
        if novo_nome != "":
            produto.nome = novo_nome
        if novo_preço != "":
            produto.preco = float(novo_preço)
...
def listar(motor):
    ...
    with Session(motor) as sessao:
        ...
        for produto in rset:
            ...
            print(f" {produto.nome[:40]:40s}  {produto.preco:10.2f}  {produto.estoque:7d}     "f"{ativo}     {produto.categoria.nome}")
...
def vender(motor):
    ...
    with Session(motor) as sessao:
        ...
        unidades = int(input("Vendereremos quantas? "))
